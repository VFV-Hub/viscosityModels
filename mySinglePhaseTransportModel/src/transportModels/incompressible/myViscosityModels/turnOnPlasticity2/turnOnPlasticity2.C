...
Foam::myViscosityModels::turnOnPlasticity2::calcNu() const
{
...
Foam::tmp<Foam::volScalarField>
Foam::myViscosityModels::turnOnPlasticity2::calcTauY() const
{
	dimensionedScalar TVSMALL("VSMALL", dimTemperature, VSMALL);
	const volScalarField& T=U_.mesh().lookupObject<volScalarField>("T");
...
Foam::myViscosityModels::turnOnPlasticity2::turnOnPlasticity2
(
    const word& name,
    const word& nameDois,
...
:
    myViscosityModel(name, nameDois, viscosityProperties, U, phi),
...
bool Foam::myViscosityModels::turnOnPlasticity2::read
(
    const dictionary& viscosityProperties
)
{
    myViscosityModel::read(viscosityProperties);
